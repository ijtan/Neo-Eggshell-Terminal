...
int Executor(vector<string> &argVector, string &line, vector<int> conf)
{
    ...
    if ((internalChecker(argVector[0]) == 1) && (conf[0] == 1 || conf[1] == 1 || conf[2] == 1))
    { //if command is ParentOnly but there are redirects -> abort (pipe checks itself)
        cout << "One or more internal commands entered are non rediractable. Aborting..." << endl;
        return 0;
    }
    else if ((internalChecker(argVector[0]) == 1) && !(conf[0] == 1 || conf[1] == 1 || conf[2] == 1 || conf[3] == 1))
    { //could simply check for (conf[1..3]==0) same thing.
